CREATE TABLE `ra_drifting_events`
(
...
CREATE TABLE `ra_drifting_races`
(
    `id`                    INT(11)     NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `ra_drifting_tracks`
(
    `id`                 INT(11)                   NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `ra_drifting_results`
(
    `id`                 INT(11)          NOT NULL AUTO_INCREMENT,
    ...
    `best_score`         INT(11) UNSIGNED NOT NULL DEFAULT 0,
    ...
    PRIMARY KEY (`id`),
    INDEX `FK_ra_drifting_results_ra_drifting_races` (`race_id`),
    INDEX `FK_ra_drifting_results_ra_drifting_tracks` (`track_id`),
    CONSTRAINT `FK_ra_drifting_results_ra_drifting_races` FOREIGN KEY (`race_id`) REFERENCES `ra_drifting_races` (`id`),
    CONSTRAINT `FK_ra_drifting_results_ra_drifting_tracks` FOREIGN KEY (`track_id`) REFERENCES `ra_drifting_tracks` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION
);

CREATE TABLE `ra_drifting_user_settings`
(
    `player_identifier` VARCHAR(60)  NOT NULL,
...
INSERT INTO `ra_drifting_user_settings` (`player_identifier`, `alias`, `profile_picture`, `rating`)
VALUES ('0', 'Anonymous', 'https://i.imgur.com/Lu9dGJH.png', 1400);