...
onNet("rahe-drifting:server:calculateRatings", (raceId, players) => {
    ...
    let query = 'UPDATE ra_drifting_user_settings SET rating = ? WHERE player_identifier = ?';
    exports.oxmysql.prepare(query, queryParameters, () => {
        emit('rahe-drifting:server:ratingsUpdated', raceId, changedPlayers);
    });
});
