...
-- SERVER export - This function is exported, so it can be opened from any other server-side script by using 'exports['rahe-drifting']:openDriftingTablet(source)'
function openDriftingTablet(playerId)
    TriggerClientEvent('rahe-drifting:client:openTablet', playerId)
end

function notifyPlayer(playerId, message, type)
    TriggerClientEvent('rahe-drifting:client:notify', playerId, message, type)
end
