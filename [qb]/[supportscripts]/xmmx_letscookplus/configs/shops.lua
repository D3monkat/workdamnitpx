...
Shop = {
    ...
    GroceryBags = false, -- true if using the 3 Grocery Bags in the shop that gives players random items. If not, then false. (Check README.md)
    GroceryAmt = math.random(1, 5), -- Set the amount as randoms or single number for each item.

    enableShop = false, -- works for "qb", "qs", and "ox_inventory". If using other Inventories then you must implement your own.
    ...
    enableBlip = false, -- Must enable shop above first.
    blips = {
        {
