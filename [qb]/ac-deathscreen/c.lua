local isDead = false
RegisterNetEvent('ac-deathscreen:openUI', function(time)
    isDead = true
...
RegisterNetEvent('ac-deathscreen:revive', function()
    isDead = false
...
RegisterNetEvent('ac-deathscreen:die', function(time)
    isDead = true
    SendNUIMessage({action = "DSMenu", open = true, time = time})
    if Config.Blur then
        TriggerScreenblurFadeIn()
...
RegisterNetEvent('ac-deathscreen:res', function()
    SendNUIMessage({action = "res"})
end)

RegisterNetEvent('ac-deathscreen:updateRes', function(time)
    if time then
