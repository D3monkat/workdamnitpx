...
RegisterNetEvent('px-head2head:server:setupRace', function(citizenId, racerName, startCoords, amount, type, waypoint)
    ...
    if finishCoords then
        ...
        if Config.SoloRace then
            TriggerEvent('px-head2head:server:startRace', raceId) -- Used for debugging
        else
...
RegisterNetEvent('px-head2head:server:startRace', function(raceId)
    if useDebug then
        print('starting race')
    end
    ...
    for citizenId, racer in pairs(activeRaces[raceId].racers) do
        ...
        if Player ~= nil then
            ...
            if activeRaces[raceId].type == 'head2head' then
                TriggerClientEvent('px-head2head:client:raceCountdown', Player.PlayerData.source, activeRaces[raceId])
            else
...
RegisterNetEvent('px-head2head:server:raceStarted', function(raceId)
    activeRaces[raceId].started = true
end)

RegisterNetEvent('px-head2head:server:joinRace', function(citizenId, racerName, raceId)
    ...
    elseif activeRaces[raceId].amount > 0 and Player.PlayerData.money[Config.MoneyType] < activeRaces[raceId].amount then
        ...
        if #activeRaces[raceId].racers > 1 then
            TriggerEvent('px-head2head:server:startRace', raceId)
        end
    end
end)

RegisterNetEvent('px-head2head:server:outrunWinner', function(raceId, citizenId, opponentSource, finishTime)
    if useDebug then
...
RegisterNetEvent('px-head2head:server:finishRacer', function(raceId, citizenId, finishTime)
    if useDebug then
        print('finishing', citizenId, 'in race', raceId)
    end
    if activeRaces[raceId].winner == nil then
        activeRaces[raceId].winner = citizenId
        TriggerClientEvent('px-head2head:client:notifyFinish', source, Lang:t('info.winner'))
        if activeRaces[raceId].amount > 0 then
            local Player = QBCore.Functions.GetPlayerByCitizenId(citizenId)
        ...
        TriggerClientEvent('px-head2head:client:notifyFinish', source, Lang:t('info.loser'))
    end
end)

QBCore.Functions.CreateCallback('px-head2head:server:getPlayers', function(_, cb)
...
QBCore.Commands.Add('impsetup', 'setup impromtu',{}, false, function(source)
    TriggerClientEvent('px-head2head:client:setupRace', source)
end, 'admin')

QBCore.Commands.Add('impjoin', 'join impromtu',{}, false, function(source)
    TriggerClientEvent('px-head2head:client:joinRace', source)
end, 'admin')

QBCore.Commands.Add('impdebug', 'debug impromtu',{}, false, function(source)
    TriggerClientEvent('px-head2head:client:debugMap', source)
end, 'admin')
...
QBCore.Commands.Add('cwdebughead2head', 'toggle debug for head2head', {}, true, function(source, args)
    ...
    TriggerClientEvent('px-head2head:client:toggleDebug',source, useDebug)
end, 'admin')