...
local function handleMouse()
    ...
    if distance == 0 then
        ...
        if not hasFinished and Config.Outrun.TimeToOutrun <= GetTimeDifference(GetCloudTimeAsInt(), winTimer) then
            hasFinished = true
            TriggerServerEvent('px-head2head:server:outrunWinner', currentRace.raceId, QBCore.Functions.GetPlayerData().citizenid, opponentId, GetTimeDifference(GetCloudTimeAsInt(), startTime) )
        end
    else
        winTimer = nil
    end
...
RegisterNetEvent('cw-outrun:client:setupRace', function(data)
    ...
    TriggerServerEvent('px-head2head:server:setupRace', citizenId, racerName, startCoords, amount, 'outrun')
    handleHighBeams()
    role = 'mouse'
end)
