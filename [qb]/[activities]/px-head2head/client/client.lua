...
RegisterNetEvent('px-head2head:client:notifyFinish', function(text)
    SendNUIMessage({
        action = "Finish",
        data = {
            value = text
            ...
            if currentRace ~= nil then
                ...
                if currentRace.started and not currentRace.finished then
                    ...
                    if not hasFinished and distanceToFinish < maxDistance then
                        ...
                        TriggerServerEvent('px-head2head:server:finishRacer', currentRace.raceId, QBCore.Functions.GetPlayerData().citizenid, GetTimeDifference(GetCloudTimeAsInt(), startTime) )
                        hasFinished = true
                        Countdown = 5
                        Wait(1000)
                        finishRace()
...
RegisterNetEvent('px-head2head:client:checkDistance', function(raceId, coords, amount, host)
...
RegisterNetEvent('px-head2head:client:joinRace', function()
    if useDebug then
    ...
    if inviteRaceId then
        local citizenId = QBCore.Functions.GetPlayerData().citizenid
        local racerName = ''
        TriggerServerEvent('px-head2head:server:joinRace', citizenId, racerName, inviteRaceId)
        handleHighBeams()
    else
...
RegisterNetEvent('px-head2head:client:setupRace', function(data)
    if useDebug then
    ...
    TriggerServerEvent('px-head2head:server:setupRace', citizenId, racerName, startCoords, amount, 'head2head', finishCoords)
    handleHighBeams()
end)

RegisterNetEvent('px-head2head:client:raceCountdown', function(race)
    QBCore.Functions.TriggerCallback('px-head2head:server:getPlayers', function(players)
        Players = players
        currentRace = race
        hasFinished = false
        if useDebug then
        ...
        if currentRace.raceId ~= nil then
            ...
            TriggerServerEvent('px-head2head:server:raceStarted', currentRace.raceId)
...
local items = {
    {
        ...
        icon = 'check',
        type = 'client',
        event = 'px-head2head:client:joinRace',
        shouldClose = true
    },
    {
        id = 'head2headFree',
        ...
        icon = 'comments',
        type = 'client',
        event = 'px-head2head:client:setupRace',
        value = 0,
        shouldClose = true
    }
}
...
RegisterNetEvent('px-head2head:client:debugMap', function()
    if #markers > 0 then
...
RegisterNetEvent('px-head2head:client:toggleDebug', function(debug)
