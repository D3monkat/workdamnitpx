...
$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.action == "setupLocations") {
            if(data.ApartmentNames){
                setupNewLocations(data.locations, data.houses, data.Apartment, data.ApartmentNames, data.Access)
            }else{
                setupNewLocations2(data.locations, data.houses, data.Apartment, data.Access)
            }
        }

        if (data.action == "setupAppartements") {
            setupApps(data.locations, data.qbps)
        }
...
function setupNewLocations2(locations, myHouses, Apartment, Access) {
    var parent = $('.spawn-locations-new')
    $(parent).html("");
    $('.dropdown-menu').html("");
    ...
    setTimeout(function(){
        if(Access.apartments == true){
            $.each(Apartment, function(k, v){
                if(v.pos.pos !== undefined){
                    $(parent).append('<i style="top:'+v.pos.pos.top+'%; left:'+v.pos.pos.left+'%;" data-location="'+v.pos.label+'" data-type="appartment2" data-label="'+v.pos.label+'" data-apartname="'+v.key+'" id="IconClassStyle" class="fas fa-building IconClassStyle2"></i>')
                }
            });
        }
        ...
        if(Access.houses == true){
            if (myHouses != undefined) {
                $.each(myHouses, function(index, house){    
                    if(house.house.apartment == undefined){       
                        $(".dropdown-menu").append('<li id="IconClassStyle" data-location="'+house.house.property_id+'" data-type="house" data-label="'+house.label+'"><p><span id="'+house.house+'">'+house.label+'</span></p></li>');
                    }
                });
            }
        }
...
function setupApps(apps, qbps) {
    ...
    $.each(apps, function(index, app){
        if(app.pos !== undefined){
            if (qbps){
                $(parent).append('<i style="top:'+app.pos.top+'%; left:'+app.pos.left+'%;" data-location="'+app.label+'" data-type="appartment" data-label="'+app.label+'" id="IconClassStyle" class="fas fa-building IconClassStyle2"></i>')
            }else{
